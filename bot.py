...
def test_proxy(proxy, proxy_type="http"):
    try:
        ...
        if r.status_code == 200:
            return True, r.json().get("origin", "Unknown")
    except:
        pass
    return False, None
...
def webhook():
    ...
    if text == "/start":
        ...
        if proxies:
            lines = "🟢 SOCKS5 Proxy List:\n\n"
            for i, p in enumerate(proxies, 1):
                lines += f"{i}. <code>{p}</code>\n"
            lines += "\n📞 برای اضافه کردن به تلگرام:"
            lines += "\nSettings → Data → Proxy → Add"
        ...
        for proxy in test_list:
            ...
            if ok:
                fast.append(f"🟢 <code>{proxy}</code> → {ip}")
                if len(fast) >= 5:
                    break
        if fast:
